        ...
        <div className="overflow-x-auto relative w-full">
                    ...
                    {orders && orders.map((order: Order, index) => (
                        <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-200'}>
                            ...
                            <td className='py-2 px-4'>
                                <button className='text-blue-600 text-lg' onClick={() => editBtn(order.id, order.username, order.productName, order.status)}><FaPenToSquare /></button>
                            </td>
                        </tr>
                    ))}
            ...
            {modalVisibility && (
                <div className='fixed top-0 left-0 w-screen h-screen z-50 bg-black bg-opacity-25 flex items-center justify-center'>
                    <div className='flex flex-col items-center bg-white rounded gap-3 w-[400px] h-[250px]'>
                        <p className='my-10 text-xl'>Change the status</p>
                        <select name="" id="" onChange={(e) => setOrderStatus(e.target.value)} className='border py-1 px-2 rounded mb-5'>
                        ...
                        <div className='flex justify-around w-full'>
                            <button onClick={modalUpdateBtn} className='border border-blue-600 bg-blue-600 text-white w-32 py-1 px-2 rounded hover:bg-white hover:text-blue-600  duration-300'>Confirm</button>
                            <button onClick={modalCancelBtn} className='border border-blue-600 text-blue-600 w-32 py-1 px-2 rounded hover:bg-blue-600 hover:text-white duration-300'>Cancel</button>
                        </div>
                    </div>
                </div>
            )}
